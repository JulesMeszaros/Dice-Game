...
function CharacterCreation:new()
  ...
  --Tentative de chargement des données de personnage si elles existent
  local name = "Lionnel.le"
  if G.saveManager.data.profile then
    name = G.saveManager.data.profile.name
    print(name)
  end
  --Text en haut
  self.welcomeText = UI.Text.TextWavy:new("Create your avatar !", self.canvas:getWidth() / 2, 100, {
    amplitude = 2,
    speed = 3,
    spacing = 0.3,
    font = Fonts.soraBig,
  ...
  self.nameInput = TextInput:new(
    ...
    900,
    90,
    { noSpace = false, maxChars = 10, noSpecial = true, forceCaps = false, defaultText = name }
  )
  ...
  if G.saveManager.data.profile then
    print(G.saveManager.data.profile.avatar.head)
    self.lion:createFromIndexes(G.saveManager.data.profile.avatar)
    self.lion:update()
    self.lion:updateSprite()
  end

  print(self.lion.headIndex)
...
function CharacterCreation:updateCanvas(dt)
  ...
  for i, text in next, self.customizationTexts do
    love.graphics.draw(text[1], text[2] + px, text[3] + py, 0, 1, 1, text[1]:getWidth() / 2, text[1]:getHeight() / 2)
  end
