...
function Infos:new(run)
  ...
  self.uiElements.buttons["menuButton"] = Button:new(
    ...
    self.gameCanvas,
    function()
      return Inputs.getMouseInCanvas(0, 0, 2)
    end
  )
...
function Infos:drawGridLarge()
  ...
  for figure, i in next, Constants.FIGURES do
    --Playcount
    local playcountText =
      love.graphics.newText(Fonts.soraGridL, "Played : " .. tostring(self.run.figuresInfos[i].playcount))
    love.graphics.draw(
      playcountText,
      355,
      70 * (i - 1) + 125,
      ...
      1,
      1,
      playcountText:getWidth() / 2,
      playcountText:getHeight() / 2
    )
    --Level
    ...
    love.graphics.draw(
      ...
      1,
      1,
      basePoints:getWidth() / 2,
      basePoints:getHeight() / 2
    )
  end
  love.graphics.setColor(1, 1, 1, 1)
...
function Infos:drawBadgeHorizontal()
  ...
  if self.run.currentState == Constants.RUN_STATES.ROUND then
    ...
    local jobDeskText = love.graphics.newText(
      Fonts.soraLightMini,
      "Office " .. tostring(self.run.currentRound.deskNumber) .. " - " .. tostring(self.run.currentRound.enemyJob)
    )
    local targetText =
      love.graphics.newText(Fonts.soraReward, "Target : " .. tostring(self.run.currentRound.targetScore) .. "pts")

    love.graphics.draw(
      jobDeskText,
      self.badgeHorizontal:getWidth() / 2,
      85,
      0,
      1,
      1,
...
function Infos:drawRoundDetails(dt)
  ...
  local currentRoundText =
    love.graphics.newText(Fonts.soraSmall, "Floor " .. tostring(self.run.floorNumber) .. "\nDesk : " .. tostring("-"))
...
function Infos:fadeOut()
  self.animator:add("opacity", self.opacity, self.baseOpacity, 0.1, nil, function()
