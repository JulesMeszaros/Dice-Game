...
function CiggieObject:trigger(screen, screenType)
  ...
  then
    ...
    if G.saveManager.data["stats"]["wands"][self.id] then
      G.saveManager.data["stats"]["wands"][self.id] = G.saveManager.data["stats"]["wands"][self.id] + 1
    else
      G.saveManager.data["stats"]["wands"][self.id] = 1
    end
