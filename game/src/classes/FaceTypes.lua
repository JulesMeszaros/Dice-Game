...
function Patience:getDescription(run)
  return "[[+" .. tostring(self.pointsValue) .. "pts]]. Gains [[+20pts]] on each trigger."
end
...
function DataDice:getDescription(run)
  return "[[+"
    ...
    .. "pts]]. Gains [[+20pts]] when scored in a --numbered figure--, [[-10pts]] otherwise."
end
...
function MirrorDice:buildTriggerEffects(round)
  ...
  if sameDices > 1 then
    table.insert(effects, {
      type = "upgrade",
      fn = function()
        upgradeStat(self, "pointsValue", (sameDices - 1) * 15)
      end,
    })
  end
...
function NegativeDice:getDescription(run)
  return "Multiplies the score by the number of empty spaces in the played hand."
end
...
function BountyHunter:getDescription(run)
  if run.currentState == Constants.RUN_STATES.ROUND then
    ...
    return "[[+10pts]]. ££+15$££ if scored in a --" .. figure .. "--. (selected --figure-- changes each ++turn++)"
  else
...
function DiamondDice:buildTriggerEffects(round)
  return {
    ...
    },
    {
      type = "money",
      fn = function()
        addMoney(round, math.min(20, math.floor(round.run.money / 5)), self)
      end,
    },
  }
end
