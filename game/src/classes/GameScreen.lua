...
function GameScreen:new(floor, run, screenType, round)
  ...
  --Wavy Texts
  self.sellCiggieText = UI.Text.TextWavy:new("Sell (" .. tostring(Constants.BASE_CIGGIE_SELL_PRICE) .. "$)", 250, 950, {
    font = Fonts.soraBig,
    centered = true,
    amplitude = 5,
    speed = 2,
  ...
  self.menuBtnTX, self.menuBtnTY, self.menuBtnX, self.menuBtnY = 1555, 960 + 45, self.canvas:getWidth() + 200, 960 + 45
  ...
  self.rerollShopTX, self.rerollShopTY, self.rerollShopX, self.rerollShopY = 550 + (340 / 2), 380, 550 + (340 / 2), 380
  ...
  --Permet de mettre à jour le nbr de rerolls et de mains pour la premiere main si le nombre de rerolls de base a augmenté depuis la création de cet objet...
  if self.screenType == Constants.RUN_STATES.ROUND then
    self.animator:addDelay(0.1, function()
      ...
      self.round.remainingHands = self.run.baseHands
    end)
  end
...
function GameScreen:drawFigureGrid()
  ...
  for i = 1, 13 do
    if (self.run.currentState ~= Constants.RUN_STATES.ROUND) or not self.run.currentRound.disabledFigures[i] then
...
function GameScreen:addAvailableHand(i)
  if i ~= 7 and self.run.baseAvailableHands[i] < self.run.maxFiguresPossible then
