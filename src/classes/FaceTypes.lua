...
function MassiveDice:new(faceValue, pointsValue)
	...
	--Numbered status
	self.faceValue = faceValue --This is the face represented by the face (the number shown)
	self.pointsValue = 80 --This is the points scored by the dice
	self.totalTriggered = 0
	return self
end
	...
	return {
		{
			...
			fn = function()
				...
				if leftDice then
					round:injectDiceEffectsAtFront(leftDice)
				end
			end,
		},
	}
...
function Apparition:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function BlackStar:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function ClockWorkDice:new(faceValue, pointsValue)
	...
	self.tier = "Common"
	self.description =
		"[[+30pts]] multiplied by this face's number. Decreases the face number by one the first time you score this Face in an Office."
...
function ClockWorkDice:buildTriggerEffects(round)
	local effects = {
		{
			type = "score",
			fn = function()
				addScore(round, self.pointsValue * self.faceValue, self)
			end,
		},
	}
...
function AshtrayDice:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function SteelDice:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
				addScore(round, math.max(0, 15 - round.run.money) * 10, self)
			end,
		},
	}
end
...
function SteelDice:getDescription(run)
	return "[[+10pts]] for each $ under 10$ (currently : [[+" .. (math.max(0, 10 - run.money) * 15) .. "pts]])"
end
...
function DoubleDown:new(faceValue, pointsValue)
	...
	self.id = 16
	self.tier = "Common"
	self.description = "[[+40pts]] per even dices in scored hand"
...
function DoubleDown:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function OddJob:new(faceValue, pointsValue)
	...
	self.id = 15
	self.tier = "Common"
	self.description = "[[+40pts]] per odd dices in scored hand"
...
function OddJob:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function MusicDice:new(faceValue, pointsValue)
	...
	self.id = 17
	self.tier = "Common"
	self.description = "[[+10pts]], (X4)) if played hand contains exactly 4 dices"
...
function Signature:new(faceValue, pointsValue)
	...
	self.id = 18
	self.tier = "Common"
	self.description = "[[+10pts]]. ((X3)) if this dice is the only Signature payed in this hand."
...
function Signature:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function Signature:buildUniqueEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function SniperDice:buildTriggerEffects(round)
	return {}
end
...
function Spotlight:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function Spotlight:buildFirstEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function RiskyBusiness:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function Patience:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function DataDice:new(faceValue, pointsValue)
	...
	self.spriteSheet:setFilter("linear", "linear")
	self.description =
		"Scoring : [[+10pts]], increases by [[20pts]] if figure is a numbered figure (1, 2, 3,...) decreases by [[10pts]] if not."
...
function DataDice:buildTriggerEffects(round)
	local effects = {}
...
function StockOption:buildTriggerEffects(round)
	local effects = {}
...
function RainbowDice:new(faceValue, pointsValue)
	...
	self.spriteSheet:setFilter("linear", "linear")
	self.description = "Scoring : [[+10pts]], goes up by 50pts if played in a small or large straight."
...
function RainbowDice:buildTriggerEffects(round)
	local effects = {}
...
function RainbowDice:getDescription(run)
	return "Scoring : [[+"
		.. tostring(self.pointsValue)
		.. "pts]], goes up by 50pts if played in a small or large straight."
end
...
function MagicDice:buildTriggerEffects(round)
	local effects = {}
...
function ReturnOnInvestment:buildBackupEffects(round)
	local i = math.floor(round.run.money / 15)
	local f = i * 1.5
...
function ReturnOnInvestment:getDescription(run)
	return "Backup : Multiplies the score by ((1.5)) for each 15$ in bank (currently : (("
...
function LoyaltyCard:buildTriggerEffects(round)
	return {
		{
			type = "money",
			fn = function()
...
function MirrorDice:new(faceValue, pointsValue)
	...
	self.spriteSheet:setFilter("linear", "linear")
	self.description = "Scoring : [[+10pts]], gains [[10pts]] by played dice with the same number as this one"
...
function MirrorDice:buildTriggerEffects(round)
	local effects = {}

	local sameDices = 0
	local sortedFaces, _ = round:getDicesOrder(round.usedDices)
	for _, f in next, sortedFaces do
		if f.representedObject.faceValue == self.faceValue or f.representedObject.blank == true then
...
function MirrorDice:getDescription(run)
	return "Scoring : [[+"
		.. tostring(self.pointsValue)
		.. "pts]], gains [[10pts]] by played dice with the same number as this one"
end
...
function CookieDice:buildTriggerEffects(round)
	local effects = {}
...
function InsomniacDice:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
				addScore(round, 10 + (20 * round.run.totalUsedCoffees), self)
			end,
		},
	}
end
...
function InsomniacDice:getDescription(run)
	return "Scoring : [[+"
		...
		.. "pts]], goes up by [[20pts]] for each coffee used in this building."
end
...
function TwinFlame:buildTriggerEffects(round)
	local effects = {
		{
			type = "score",
			fn = function()
	...
	table.insert(effects, {
		...
		fn = function()
			if
				table.getn(round.run.ciggiesObjects) < round.run.maxCiggies
...
function FaxMachine:buildTriggerEffects(round)
	local _, dicesOrder = round:getDicesOrder(round.usedDices)
...
function Necromancer:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function Necromancer:getPointsValue(run)
	return 45 * run.totalDisabled
end

function Necromancer:getDescription(run)
	return "Scoring : [[+"
		.. self:getPointsValue(run)
		.. "pts]]. Goes up by [[45pts]] each time a dice got disabled in this office"
end
...
function SixthSense:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function SixthSense:getDescription(run)
	...
	return "Scoring : [[+"
		...
		.. "pts]]. Goes up by [[50pts]] for each dice face in deck currently disabled. (currently : (("
		.. tostring(i * 50)
		.. "pts)))"
end
...
function Sacrifice:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function CheckeredDice:buildTriggerEffects(round)
	local effects = {
		{
			type = "score",
			fn = function()
...
function StarDice:new(faceValue, pointsValue)
	...
	self.description = "Scoring : [[+70pts]]. Can be counted as any number for numbered figures."
	--Numbered status
	self.faceValue = faceValue --This is the face represented by the face (the number shown)
...
function StarDice:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function Adrenaline:buildTriggerEffects(round)
	local effects = {
		{
			type = "score",
			fn = function()
...
function Adrenaline:getDescription(run)
	return "Scoring : [[+" .. self:getPointsValue(run) .. "]]. If this is the last turn, ((X5))."
end
...
function NegativeDice:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
				local facesOrder, _ = round:getDicesOrder(round.usedDices)
				local i = 5
...
function BountyHunter:buildTriggerEffects(round)
	local effects = {
		{
			type = "score",
			fn = function()
	...
	local shouldAddMoney = false
	if round.playedFigure <= 6 then
		shouldAddMoney = true
	elseif round.playedFigure == round.bountyHunterFigure then
		shouldAddMoney = true
	end

	if shouldAddMoney then
		table.insert(effects, {
			type = "money",
			fn = function()
				addMoney(round, 15, self)
			end,
		})
	end
...
function BountyHunter:getDescription(run)
	if run.currentState == Constants.RUN_STATES.ROUND then
		local figure = Constants.FIGURES_LABELS[run.currentRound.bountyHunterFigure]
		if run.currentRound.bountyHunterFigure < 7 then
			figure = "Numbered Figure"
		end

		return "Scoring : +15$ if scored in a " .. figure .. "."
	else
		return "Scoring : Adds 15$ if scored in a selected figure (selected figure changes each hand)"
	end
end
...
function Upcycling:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function Ectoplasm:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
				addScore(round, 200, self)
			end,
		},
	}
end
...
function DieAndRetry:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function InvisibleHand:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
				addScore(round, round.run.totalspent, self)
			end,
		},
	}
end
...
function UndeadDice:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function AllIn:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function Poltergeist:buildTriggerEffects(round)
	local effects = {
		{
			type = "score",
			fn = function()
				addScore(round, 20, self)
			end,
		},
	}
...
function FortuneDice:buildTriggerEffects(round)
	...
	table.insert(effects, {
		type = "mult",
		fn = function()
			multiplyScore(round, math.max(1, 1.5 * math.floor(round.run.money / 15)), self)
		end,
	})
...
function FortuneDice:getDescription(run)
	return "Adds a Fortune Magic Wand to inventory. Adds ((X0.5)) for each 15$ in bank (currently : ((X"
...
function DimeDice:new(faceValue, pointsValue)
	...
	self.tier = "Common"
	self.id = 53
	self.description = "[[+10pts]]. +4$ per played dice with the same number as this one."
...
function DimeDice:buildTriggerEffects(round)
	local i = 0
...
function CapitalistDice:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
				addScore(round, math.floor(round.run.money * 3), self)
			end,
		},
	}
end
...
function CapitalistDice:getDescription(run)
	return "[[+3pts]] par $ in bank (currently : [[+" .. tostring(math.floor(run.money * 3)) .. "pts]])"
end
...
function LuckyStar:buildTriggerEffects(round)
	local effects = {
		{
			type = "score",
			fn = function()
...
function Doom:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function Godspeed:buildTriggerEffects(round)
...
function WitchDice:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
				addScore(round, round.usedCiggiesRound * 50, self)
			end,
		},
	}
end
...
function WizardDice:buildTriggerEffects(round)
	return {
		{
			type = "mult",
			fn = function()
...
function WizardDice:getDescription(run)
	return "[[X1.5pts]] for each Magic Wand Used in this office. (Currently : X"
...
function RecursiveDice:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
				local i = round.run.figuresInfos[round.playedFigure].playcount
				addScore(round, i * 20, self)
			end,
		},
	}
end
...
function Echo:buildTriggerEffects(round)
	...
	table.insert(effects, {
		type = "score",
		fn = function()
			addScore(round, 50 * self.consecutiveCount, self)
		end,
	})
...
function CrossedOut:buildTriggerEffects(round)
	return {
		{
			type = "score",
			fn = function()
...
function TimeDice:buildTriggerEffects(round)
	local effects = {}
	...
	table.insert(effects, {
		...
		fn = function()
			...
			multiplyScore(round, math.max(1, 0.2 * i), self)
		end,
	})
