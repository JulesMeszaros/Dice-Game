...
function EndRound:new(run, round)
	...
	--You Won Text
	self.youWon = UI.Text.TextWavy:new("You Won!", self.contentCanvas:getWidth() / 2, 70, {
		centered = true,
...
function EndRound:updateEarnedMoney()
	...
	local coworkerDollars = love.graphics.newText(
		Fonts.soraReward,
		{ { 255 / 255, 178 / 255, 89 / 255 }, string.rep("$", self.round.baseReward) }
	)
	local turnsDollars = love.graphics.newText(
		Fonts.soraReward,
...
function EndRound:mousemoved(x, y, dx, dy, isDragging)
	...
	if isDragging == true then
		...
		if self.ciggieReward and self.ciggieReward.isDraggable and self.ciggieReward.isBeingClicked then
...
function EndRound:getCurrentlyHoveredObject()
	...
	--Ciggie
	if self.ciggieReward and self.ciggieReward:isHovered() then
		self.currentlyHoveredFace = self.ciggieReward
		return
	end
end
...
function EndRound:outAnimation()
	...
	for i, uiFace in next, self.faceRewards do
		uiFace.animator:addGroup({
			--Scale
			{
				property = "targetY",
				from = uiFace.targetY,
				targetValue = self.canvas:getHeight() + 150,
				duration = 0.3,
	...
	self.ciggieReward.animator:addGroup({
		...
		{
			property = "targetY",
			from = self.ciggieReward.targetY,
			targetValue = self.canvas:getHeight() + 150,
			duration = 0.3,
			easing = AnimationUtils.Easing.inCubic,
		},
	...
	self.animator:addDelay(0.2, function()
		self.round.terrain:outAnimation()
	end)
end
