...
function DiceCustomization:new(previousRound, newFaceObjects)
	local self = setmetatable(
		Screen:new(
			previousRound.run.currentFloor,
			previousRound.run,
			Constants.RUN_STATES.DICE_CUSTOMIZATION,
			previousRound
		),
		DiceCustomization
	)
...
function DiceCustomization:mousereleased(x, y, button, istouch, presses)
	...
	for i, face in next, self.rewardsUIFaces do
		...
		elseif
			(
				face.targetX > self.inventoryMDTX
				and face.targetX < self.inventoryMDTX + self.inventoryCanvasMedium:getWidth()
			)
			and (
				face.targetY > self.inventoryMDTY
				and face.targetY < self.inventoryMDTY + self.inventoryCanvasMedium:getHeight()
			) --[[ and
			...
			if face.isBeingDragged == true then
				...
				if closestFace then
					...
					if isAlreadyCovered == false then
						...
						local diceObjectToSwap =
							self.uiDices[closestFace[3]][closestFace[4]].representedObject.diceObject
...
function DiceCustomization:outAnimation()
	...
	self.animator:addGroup({
...

		{
			...
			targetValue = 0 - self.customizationMat:getHeight() - 50,
			duration = outDuration,
			easing = AnimationUtils.Easing.inOutCubic,
		},
		{
		...
		{
			property = "inventoryMDY",
			from = self.inventoryMDY,
			targetValue = -50 + self.rewardsMediumCanvas:getHeight() - self.customizationMat:getHeight(),
			duration = outDuration,
			easing = AnimationUtils.Easing.inOutCubic,
		},
		{
			property = "rewardsMDY",
			from = self.rewardsMDY,
			targetValue = -50 - self.customizationMat:getHeight(),
			duration = outDuration,
			easing = AnimationUtils.Easing.inOutCubic,
		},
		{
		...
		for j, face in next, dice do
			...
			face.animator:addGroup({
				...
				},
				{
					property = "rotation",
					from = face.rotation,
					targetValue = randomAngle,
					duration = duration,
					easing = AnimationUtils.Easing.inCubic,
				},
				{
					property = "baseRotation",
					from = face.baseRotation,
					targetValue = randomAngle,
					duration = duration,
					easing = AnimationUtils.Easing.inCubic,
				},
	...
	self.animator:addDelay(0.5, function()
		self.round.run:goToRoundSelection()
	end)
end
	...
	for i, face in next, self.newUIFaces do
		...
		if closestFace then
			...
			for k, d in next, self.run.facesInventory do
				if d == face.representedObject then
					table.remove(self.run.facesInventory, k)
					break
				end
			end
		end
...
function DiceCustomization:detectClosestFace(x, y)
	...
	for i = 1, 5 do --loop over dices
		...
		for j = 1, 6 do --loop over faces
			if
				math.abs((x + 60) - (relativeXPositions[j] + basisX)) < 60
				and math.abs((y + 60) - (relativeYPositions[j] + basisY)) < 60
			then
...
function DiceCustomization:createInventoryUI()
	...
	for i, face in next, self.newFaceObjects do
		local diceFace = DiceFace:new(nil, face, startX, startY, 120, false, true, function()
		...
		diceFace.animator:addGroup({
			{
				property = "x",
				from = startX,
				targetValue = self.xPositions[i] + self.inventoryMDTX + 60,
				duration = duration,
			...
			},
			{
				property = "y",
				from = startY,
				targetValue = self.yPositions[i] + self.inventoryMDTY + 60,
				duration = duration,
				easing = AnimationUtils.Easing.outQuad,
			},
			{
		...
		diceFace.animator:addGroup({
			{
				property = "x",
				from = startX,
				targetValue = self.xPositionsRewards[i] + self.rewardsMDTX + 60,
				duration = duration,
				easing = AnimationUtils.Easing.outQuad,
			},
			{
			...
			},
			{
				property = "y",
				from = startY,
				targetValue = self.yPositionsRewards[i] + self.rewardsMDTY + 60,
				duration = duration,
				easing = AnimationUtils.Easing.outQuad,
			},
			{
...
function DiceCustomization:createDiceUI(diceObject, i)
	...
	for k, faceObject in next, diceObject:getAllFaces() do
		...
		local diceFace = DiceFace:new(diceObject, faceObject, startX, startY, 120, true, true, function()
			return Inputs.getMouseInCanvas(0, 0)
		end, nil)
		...
		diceFace.animator:addGroup({
			{
				property = "x",
				from = startX,
				targetValue = xOffset + relativeXPositions[k],
				duration = duration,
			...
			},
			{
				property = "y",
				from = startY,
				targetValue = yOffset + relativeYPosition[k],
				duration = duration,
				easing = AnimationUtils.Easing.outCubic,
			},
			{
...
function DiceCustomization:sellDiceFace(face, faceUI, key)
	...
	faceUI.animator:addGroup({
		...
		},
		{
			...
			easing = AnimationUtils.Easing.easeOutBack,
			onComplete = function()
				table.remove(self.newUIFaces, key)
				self:updateInventoryPositions()
			end,
		},
	})
